...
test('One Store test', () => {
  ...
  expect(forwarder).toHaveBeenCalledWith({
    store: ['todoStore', 'todo2Store'], method: 'getTodo2Store', args: ['hello', 'world']
  });
});
...
test('One Store with path test', () => {
  ...
  let resProxy = proxyHandler.proxyStores({
    winTodoStore: {
      type: "Store", filters: null, path: ["winManagerStore", {type: "Map", name: "winPackMapStore", index: "mainClient"}], 
    }
  }, forwarder);
